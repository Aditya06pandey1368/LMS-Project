...
export const createCheckoutSession = async (req, res) => {
  try {
    ...
    newPurchase.paymentId = session.id; // ✅ save session ID
...
export const stripeWebhook = async (req, res) => {
  ...
  try {
    if (event.type === "checkout.session.completed") {
      ...
      const purchase = await CoursePurchase.findOne({
        paymentId: session.id, // ✅ must match createCheckoutSession
      }).populate("courseId");

      if (!purchase) {
        console.error("❌ Purchase not found for session", session.id);
        return res.status(200).end(); // always return 200
      }
  ...
  res.status(200).end(); // ✅ Stripe requires plain 200 response
};
...
export const getCourseDetailWithPurchaseStatus = async (req, res) => {
    try {
        ...
        // 🔥 FIX: Only find a purchase if the status is 'completed'
        ...
        if (!purchased) {
            return res.status(200).json({
                course,
                purchased: false // Ensures frontend knows it's not bought yet
            });
        }
